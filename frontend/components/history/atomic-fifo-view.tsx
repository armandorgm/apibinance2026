...
/**
 * AtomicFifoView — Vista "Binomial / Dumbbell"
 *
 * Filosofía: Cada trade es un par perfecto (1 compra × 1 venta de cantidad exacta).
 * Se representa como una línea de tiempo de pares con una barra de tensión entre
 * entrada y salida cuyo color y grosor refleja el PnL.
 */
...
import { ArrowRight, Clock, Clock3 } from 'lucide-react'
...
function DumbbellCard({ trade, index, openOrders }: { trade: Trade; index: number; openOrders?: Order[] }) {
  ...
  if (isExitOpen) {
    if (hasCond) {
      ...
      statusText = hasMarket ? 'Salida [STD / MARKET]' : 'Salida [STD / LIMIT]'; // usually limit if standard open
    } else {
      glowTheme = 'float';
            ...
            <div className="relative z-10 flex flex-col justify-center h-full">
              <div className="flex items-center gap-2 mb-1">
                ...
                {isExitOpen && glowTheme !== 'float' && (
                  <span className="text-[10px] font-bold italic opacity-60 ml-2 animate-pulse">• Waiting Fill</span>
                )}
              </div>
          ...
          <div className="flex items-center justify-end px-5 py-4 min-w-[120px] border-l border-slate-100 dark:border-slate-800 relative z-20">
            {!isExitOpen ? (
               ...
               <div className="flex flex-col items-end gap-1">
                 <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 text-right animate-pulse">Floating</span>
                 <PnlBadge pnl={trade.pnl_net} percentage={trade.pnl_percentage} />
               </div>
            )}
          </div>
...
export function AtomicFifoView({ trades, symbol }: Props) {
  const { data: openOrders } = useOpenOrders(symbol)

  // Ocultar trades que son puramente ALGO sin fill (ya que los procedurizamos en el lado PENDING)
  const realTrades = trades.filter(t => !t.is_pending);

  if (realTrades.length === 0) {
    return (
      <div className="p-12 text-center text-gray-400 dark:text-gray-500">
        <div className="text-4xl mb-3">⚖️</div>
        <p className="text-sm">No hay posiciones emparejadas con Atomic FIFO.</p>
      </div>
    )
  }
    ...
    <div className="p-6 flex flex-col gap-4">
      ...
      <style>{`
        ...
        @keyframes stepped-glow-algo {
          0%, 100% { 
            opacity: 0.3; 
            box-shadow: inset 0 0 10px rgba(251,191,36,0.1), 0 0 10px rgba(245,158,11,0.2); 
          }
          50% { 
            opacity: 1; 
            box-shadow: inset 0 0 30px rgba(251,191,36,0.6), 0 0 35px rgba(245,158,11,0.9); 
          }
        }
        .animate-stepped-glow-algo {
          animation: stepped-glow-algo 1.5s steps(5, end) infinite;
        }
        ...
        @keyframes stepped-glow-std {
          0%, 100% { 
            opacity: 0.3; 
            box-shadow: inset 0 0 10px rgba(14,165,233,0.1), 0 0 10px rgba(2,132,199,0.2); 
          }
          50% { 
            opacity: 1; 
            box-shadow: inset 0 0 30px rgba(14,165,233,0.6), 0 0 35px rgba(2,132,199,0.9); 
          }
        }
        .animate-stepped-glow-std {
          animation: stepped-glow-std 1.5s steps(5, end) infinite;
        }
        ...
        @keyframes stepped-glow-float {
          0%, 100% { 
            opacity: 0.3; 
            box-shadow: inset 0 0 10px rgba(148,163,184,0.1), 0 0 10px rgba(100,116,139,0.2); 
          }
          50% { 
            opacity: 1; 
            box-shadow: inset 0 0 30px rgba(148,163,184,0.4), 0 0 35px rgba(100,116,139,0.7); 
          }
        }
        .animate-stepped-glow-float {
          animation: stepped-glow-float 1.5s steps(5, end) infinite;
        }
      `}</style>
      ...
      <div className="flex items-center gap-6 text-[11px] text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-3">
        <span className="flex items-center gap-1.5">
          ...
          Vista Binomial — par exacto ENTRADA→SALIDA
        </span>
        <span className="flex items-center gap-1.5">
        ...
        <span className="flex items-center gap-1.5">
          ...
          Barra roja = PnL negativo
        </span>
      </div>

      {realTrades.map((trade, i) => (
        <DumbbellCard key={trade.id} trade={trade} index={i} openOrders={openOrders} />
      ))}
    </div>
  )
}
