              ...
              logs?.map((log) => (
                <div key={log.id} className="flex items-center gap-4 p-3 bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    {log.action_taken === 'NEW_ORDER' && (log.exchange_response) && (
                      <div className="mt-2 text-[10px] text-indigo-500 dark:text-indigo-400 font-mono">
                         ID Binance: {(() => {
                             try {
                               const res = JSON.parse(log.exchange_response);
                               return res.id || res.orderId || 'N/A';
                             } catch { return 'N/A'; }
                         })()}
                      </div>
                    )}
